...
fn solve(sc: &mut Scanner<io::StdinLock>) {
    let n: usize = sc.next();
    let x: u64 = sc.next();
    let v: Vec<u64> = (0..n).map(|_| sc.next()).collect();

    let mut pair_sum: HashMap<u64, (usize, usize)> = HashMap::new();
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            if s < x {
                let need = x - s;
                if let Some(&(a, b)) = pair_sum.get(&need) {
                    if a != i && a != j && b != i && b != j {
                        println!("{} {} {} {}", a + 1, b + 1, i + 1, j + 1);
                        return;
                    }
                }
            }
            pair_sum.entry(s).or_insert((i, j));
        }
    }
