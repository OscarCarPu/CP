...
fn solve(sc: &mut Scanner<io::StdinLock>) {
    let (n, x) = (sc.next(), sc.next());
    let mut v: Vec<(usize, usize)> = (0..n).map(|i| (sc.next(), i)).collect();
    v.sort();
    for i in 1..=n - 1 {
        for j in 0..i {
            if v[i].0 + v[j].0 > x {
            ...
            let need = x - v[i].0 - v[j].0;
            let (mut hi, mut lo) = (n - 1, i + 1);
